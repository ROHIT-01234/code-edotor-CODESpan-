...
const app = express();
const { JOIN, JOINED, CODE_CHANGE, USER_CHANGES, SYNC_CODE, TOGGLE_EDITOR_LOCK, DISCONNECTED } = ACTIONS;
...
const loginUser = async (req, res) => {
  ...
  const token = sign({ name: user.username }, "sfsfs");
  try {
...
io.on("connection", (socket) => {
  ...
  socket.on(CODE_CHANGE, ({ roomId }) => {
    ...
    io.in(roomId).emit(USER_CHANGES, userChanges);
  });
  ...
  socket.on(TOGGLE_EDITOR_LOCK, ({ roomId, editorLocked }) => {
    // Emit the new TOGGLE_EDITOR_LOCK action to other users in the room
    socket.to(roomId).emit(TOGGLE_EDITOR_LOCK, { editorLocked });
  });
